...
- **Enhanced Auto-Backup System (July 2025)**: Completely revamped auto-backup functionality with real-time data change detection using localStorage monitoring. System now automatically downloads updated JSON backup files to Files app when data changes occur, using consistent filename "fitcircle-auto-backup.json" that overwrites previous versions. Features debounced backup triggers (5-second delay after changes), change detection via data hashing to prevent unnecessary downloads, iOS-compatible silent downloads, and comprehensive monitoring including app visibility changes and page loads. Removed redundant secure backup section that duplicated existing Complete Data Backup functionality.
- **Auto-Backup UI Improvements (July 2025)**: Fixed toggle switch positioning issues using absolute positioning and added custom backup path functionality. Replaced history icon with folder icon that opens path selector modal, allowing users to define custom file paths for auto-backup saves in Files app. Enhanced toggle switches across all settings sections for consistent visual appearance.
